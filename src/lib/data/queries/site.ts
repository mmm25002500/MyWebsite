...
export const getSiteSettings = cached(
  ...
  async (locale: Locale): Promise<SiteSettings> => {
    ...
    return {
      ...
      socialLinks:
        (map.get('social_links') as SiteSettings['socialLinks'] | undefined) ??
 ...
 * 導覽列的顯示與順序。
 *
 * 存在 `site_settings` 的 `nav_items`，形如 `[{ key, isVisible }]`。沒有設定
 * 或設定壞掉時退回 `navItems` 的完整預設——導覽列消失比順序不對嚴重得多。
 */
export const getNavItems = cached(
  ['getNavItems'],
  async (): Promise<{ key: NavKey; href: string; isVisible: boolean }[]> => {
    const fallback = navItems.map((item) => ({ ...item, isVisible: true }));
    ...
    const known = new Map(navItems.map((item) => [item.key as string, item.href]));
    const ordered: { key: NavKey; href: string; isVisible: boolean }[] = [];

    for (const entry of stored) {
      if (typeof entry !== 'object' || entry === null) continue;
      const key = (entry as { key?: unknown }).key;
      if (typeof key !== 'string') continue;
      const href = known.get(key);
      if (!href) continue;
      ordered.push({
        key: key as NavKey,
        href,
        isVisible: (entry as { isVisible?: unknown }).isVisible !== false,
      });
      known.delete(key);
    }

    // 設定裡沒提到的項目（例如之後新增的頁面）補在最後，預設顯示。
    for (const item of navItems) {
      if (known.has(item.key)) ordered.push({ ...item, isVisible: true });
    }
...
export const getHomeSections = cached(
  ['getHomeSections'],
...
export const getOrganizations = cached(
  ['getOrganizations'],
  async (locale: Locale): Promise<Organization[]> => {
...
export const getLinkGroups = cached(
  ['getLinkGroups'],
  async (locale: Locale): Promise<LinkGroup[]> => {
...
export const getSponsorMethods = cached(
  ['getSponsorMethods'],
  async (locale: Locale): Promise<SponsorMethod[]> => {
...
export const getSponsors = cached(
  ['getSponsors'],
  async (locale: Locale): Promise<SponsorEntry[]> => {
...
export const getSiteStats = cached(
  ['getSiteStats'],
  async (locale: Locale): Promise<SiteStats> => {
